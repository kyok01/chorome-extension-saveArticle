...
//   // runtime.
//   let url = chrome.runtime.getURL("article-list.html");
...
function saveArticleInfo() {
  // id関係を実装しようとしたが、変数関係のエラーが出るので断念
  // chrome.storage.local.get(["id"], function (num) {
  //   console.log(num);
  //   if (num.id >= 1) {
  //     idNum = num.id;
  //     console.log("b");
  ...
  const articleMemo = [articleTitle, articleUrl];
  chrome.storage.local.set({ article: articleMemo }, function () {});

  chrome.storage.local.get(["article"], function (article) {
    console.log(article);
  });
  ...
  // chrome.storage.local.set({ idNum: JSON.stringify(articleMemo) }, function () {
  //   console.log("aaaaaa");
  // });
}
